...
const PurpleKnightADOverview = () => {
  ...
  const metrics = [
    { id: 'note', label: 'Note', name: 'Note', color: '#ef4444', main: true },
    { id: 'ioes-found', label: 'IOEs Found', name: 'IOEs Found', color: '#7c3aed' },
    { id: 'critical-ioes', label: 'Critical IOEs', name: 'Critical IOEs', color: '#f97316' },
  ]
  ...
  const getIndicator = (value, referenceValue, metricName) => {
    ...
    if (isInverted) {
      if (value <= referenceValue) {
        return { icon: TrendingDown, color: 'text-success', status: 'Below Target' }
      } else {
        return { icon: TrendingUp, color: 'text-destructive', status: 'Above Target' }
      }
    } else {
      if (value >= referenceValue) {
    ...
    <div className="space-y-6 mb-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {metrics.map((m) => {
          ...
          const ref = dataManager.getReferenceValue(CATEGORY, m.name)
          const indicator = getIndicator(value ?? 0, ref, m.name)
      ...
      {/* Follow-up Items Card */}
      <FollowupCard source="Purple Knight AD" />
